...
# read data
pregcare = read.csv('C:/Users/sbush/Desktop/pregcare/FAM34-downsampled.csv')
...
for (iobs in 1:nrow(IDs)) {
  ...
  # FAM34 is a delGTGGCTCT mutation
  ctrlREF = sum(controls$count_hq_G, na.rm = TRUE)
  ctrlALT = sum(controls$count_hq_DEL, na.rm = TRUE)
  ...
  for(irow in 1:nrow(dat)) {
	...
	# FAM34 is a delGTGGCTCT mutation
	caseREF = dat$count_hq_G[irow]
    caseALT = dat$count_hq_DEL[irow]
...
write.csv(res, file='C:/Users/sbush/Desktop/pregcare/FAM34-downsampled-corrected.csv', row.names = FALSE)